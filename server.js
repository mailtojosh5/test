...
const JENKINS_TOKEN = "110c32e949d42238f6c27ef3e60438defa";
...
const jobs = [
  {
    ...
    cucumber: "https://localhost:8080/job/TypeScriptHybridAllureFramework/8/execution/node/3/ws/cucumber.json"
  },
  {
    id: "job2",
    name: "API Automation",
...
app.get("/api/job/:id", async (req, res) => {
  ...
  try {
    ...
    reportRes.data.forEach(feature => {
      ...
      feature.elements.forEach(scenario => {
        ...
        if (statuses.includes("failed")) {
          ...
          if (!moduleFailures[feature.name]) {
            moduleFailures[feature.name] = 0;
          }
    ...
    const stability = {
      ...
      values: [((stats.passed / stats.total) * 100).toFixed(2)]
    };
